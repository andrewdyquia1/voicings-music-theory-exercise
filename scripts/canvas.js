...
function draw() {
  ...
  const canvasForeground = document.getElementById("canvas__foreground");
  const ctxForeground = canvasForeground.getContext("2d");

  const grandStaff = "icons/grand-staff.svg";

  function createImages(source, xCoord, yCoord, ctx) {
  ...
  function drawNotesAndSharps() {
    for (i = 0; i < notesIndex; i++) {
      const note = notesArr[i],
        sound = soundsArr[i],
        accidental = sound.sharpOrFlat;
      const sharp = "icons/sharp-sign.svg",
        flat = "icons/flat-sign.svg",
        doubleSharp = "icons/double-sharp.svg",
        wholeNote = "icons/whole-note.svg";
      const noteXCoord = note.xCoord,
        noteYCoord = note.yCoord;
      const accidentalXCoord = note.xCoord - note.xCoordModifier - 20,
        accidentalYCoord = note.yCoord - 16;
      //draws the accidental if required
      if (accidental == "sharp") {
        createImages(sharp, accidentalXCoord, accidentalYCoord, ctxForeground);
      } else if (accidental == "flat") {
        createImages(flat, accidentalXCoord, accidentalYCoord, ctxForeground);
      ...
      createImages(wholeNote, noteXCoord, noteYCoord, ctxForeground); // draws the note
    }
  }
  ...
  function drawLedgerLines() {
    for (i = 0; i < ledgerLinesArr.length; i++) {
      const ledgerLine = "icons/ledger-line.svg",
        thereIsLedger = ledgerLinesArr[i].true;
      const ledgerLineXCoord = xCoord,
        ledgerLineYCoord = ledgerLinesArr[i].yCoord;
      ...
      if (thereIsLedger) {
        createImages(
          ...
          ledgerLineYCoord,
          ctxForeground
        ); // draws the ledger line if required
      }
    }
  }

  //draw blank white canvases
  ctxBackground.clearRect(0, 0, canvasBackground.width, canvasBackground.height); 
  ctxForeground.clearRect(0, 0, canvasBackground.width, canvasBackground.height);

  createImages(grandStaff, 0, 0, ctxBackground); //draws music staff
...
function updateScreen() {
  ...
  function determineLedgerLines(note, drawn) {
    if (note == 0 && drawn) {
      //if note is C below bass clef, set the 2 lines needed to true
      ledgerLinesArr[0].true = true;
      ledgerLinesArr[1].true = true;
    } else if (note > 0 && note < 3 && drawn) {
      //if the note either D or E below bass clef, set the 1 line needed to true
      ledgerLinesArr[1].true = true;
    } else if (note == 14 && drawn) {
      //if middle C, set the 1 line needed to true
      ledgerLinesArr[2].true = true;
    } else if ((note == 26 || note == 27) && drawn) {
      //if A or B above treble staff, set 1 line needed to true
      ledgerLinesArr[3].true = true;
    } else if (note == 28 && drawn) {
      //if C above treble staff, set 2 lines need to true
      ledgerLinesArr[3].true = true;
  ...
  function repositionAccidentals() {
    for (i = 1; i < soundsArr.length; i++) {
      ...
      if (
        (soundsArr[prevIndex].interval == "m3" ||
          soundsArr[prevIndex].interval == "M3") &&
        soundsArr[prevIndex].sharpOrFlat != "none" &&
        soundsArr[currentIndex].sharpOrFlat != "none" &&
        notesArr[prevIndex].staff == notesArr[currentIndex].staff &&
        notesArr[prevIndex].isDrawn &&
        notesArr[currentIndex].isDrawn
      ) {
  ...
  function updateButtons() {
    const exerciseContainerChoices = document.querySelector(
      ".exercise__container--choices"
    );
    const currentSoundsArrIndex = notesIndex - 1;
    const currentExerciseNotesIndex = Array.prototype.indexOf.call(
      exerciseNotes,
      soundsArr[currentSoundsArrIndex].exerciseNoteItem
    );
    ...
    for (i = 1; i < possibleButtonsArr.length + 1; i++) {
      if (
        soundsArr[currentSoundsArrIndex].exerciseNoteItem ==
        possibleButtonsArr[i - 1].exerciseNoteItem
      ) {
        //if the current sound from soundsArr matches the sound of the possibleButtonsArr
    ...
    while (exerciseContainerChoices.firstChild) {
      //while exerciseContainerChoices has a firstChild
    ...
    for (i = 0; i < possibleButtonsArr.length; i++) {
      const possibleExerciseNotesIndex = Array.prototype.indexOf.call(
        exerciseNotes,
        possibleButtonsArr[i].exerciseNoteItem
      );
      possibleButtonsArr[i].interval =
        intervalsArr[possibleExerciseNotesIndex - currentExerciseNotesIndex]; //creates interval key
